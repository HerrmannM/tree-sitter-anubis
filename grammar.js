...
const TOKS = [
  ...
  [right, {
    ...
    // parenthesised term (beyond LR(1) otherwise). grammar.y:783 (`sym |-> Term`)
    // is unreachable as a result, which is correct: the parens are mandatory.
    mapsto:{tok: $=>/\)\s*\|->/},
    ...
    comment: $ => token(choice(
      ...
      par_read: $ => seq(choice(alias(/[Rr]ead/, "read"), alias(/[Tt]ransmit/, "transmit")), /[ \t\r]+/, alias(choice(/[^"]\S*/,$.lit_string), "path")),
        ...
        par_def: $=> seq(
          ...
          $.type,
          choice(
            seq($.identifier, optional(seq( alias("(", "fun"), optional($.opargs), alias(")", "fun")))),         // OpArgs allows empty
        ...
        par_type: $ => choice(
          seq(
            ...
            optional(seq(
              $.par_type_alt,
              repeat(seq(optional(alias(",", "tok")), $.par_type_alt))
            )),
        ...
        par_type_alt_factor: $ => PREC.comma(seq($.type, optional($.identifier))),
        ...
        opargs: $ => seq(
          $.operand,
          repeat(seq(optional(alias(",", "fun")), $.operand))
        ),
        ...
        conditional: $=>PREC.if(choice(
          seq("if", $.term,
            choice(
              ...
              seq( "is", choice(
                seq( alias("not","is"), field("pattern", $.term), "then", $.term, "else", $.term ), //2
          ...
          PREC.comma(seq("since", // 7
            field("subject", $.term), alias("is", "tok"),
            field("pattern", $.term), alias(",", "tok"),
            field("body", $.term)))
